...
Number ** mallocData(Number **d, int rows, int cols)
{
    ...
    // if d is NULL, realloc behaves like malloc
    p = realloc(d, size);
    if (p == NULL) {
	perror("realloc");
	exit(0);
...
{
    char *c;
    c = malloc(MATRIX_PRINT_SIZE+1);
    if (c == NULL) {
	perror("Malloc");
	exit(0);
    }

    snprintf(c, MATRIX_PRINT_SIZE, "[%d x %d Matrix]", a->rows, a->cols);
    return c;
}
...
char * printMatrix(Matrix *a)
{
    ...
    c = malloc(MATRIX_PRINT_SIZE*a->rows*a->cols);
    if (c == NULL) {
	perror("Malloc");
	exit(0);
    }
    ...
    *c = '\0';
	...
	for(i = 0; i < a->rows; i++) {
	    for (j = 0; j < a->cols; j++) {
		p1=printNumberShort(*(a->data+(a->cols*i)+j));
		strcat(c, p1);
		strcat(c, " ");
		free(p1);
	    }
...
Matrix * mulMatrixCmplx(Matrix *a, Cmplx *b)
{
    ...
    if (a == NULL) {
	fprintf(stderr, "mulMatrixCmplx(NULL)\n");
	exit(0);
    }
...
Matrix * mulMatrixReal(Matrix *a, Real *b)
{
    ...
    if (a == NULL) {
	fprintf(stderr, "mulMatrixReal(NULL)\n");
	exit(0);
    }
	...
	for(j=0; j<a->cols; j++){
	    ptr1 = a->data + (a->cols * i) + j;
	    ptr2 = p->data + (a->cols * i) + j;
	    n1 = setNumberReal(newNumber(), b);
	    *ptr2 = mulNumber(*ptr1, n1);
	    freeNumber(n1);
	}
...
Matrix * divMatrixCmplx(Matrix *a, Cmplx *b)
{
    ...
    if(a == NULL) {
	fprintf(stderr, "divMatrixCmplx(NULL)\n");
	exit(0);
    }
	...
	for (j = 0; j < a->cols; j++) {
	    ptr1 = a->data + (a->cols * i) + j;
	    ptr2 = p->data + (a->cols * i) + j;
	    n1 = setNumberCmplx(newNumber(), b);
	    *ptr2 = divNumber(*ptr1, n1);
	    freeNumber(n1);
	}
...
Matrix * divMatrixReal(Matrix *a, Real *b)
{
    ...
    if(a == NULL) {
	fprintf(stderr, "divMatrixReal(NULL)\n");
	exit(0);
    }
...
Matrix * modMatrixReal(Matrix *a, Real *b){
    ...
    if(a == NULL) {
	fprintf(stderr, "modMatrixReal(NULL)\n");
	exit(0);
    }
...
Matrix * addMatrixCmplx(Matrix *a, Cmplx *b)
{
    ...
    if(a == NULL) {
	fprintf(stderr, "addMatrixCmplx(NULL)\n");
	exit(0);
    }
...
Matrix * addMatrixReal(Matrix *a, Real *b)
{
    ...
    if(a == NULL) {
	fprintf(stderr, "addMatrixReal(NULL)\n");
	exit(0);
    }
