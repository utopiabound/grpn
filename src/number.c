...
{
    if (a == NULL) {
	fprintf(stderr, "clrRefcntNumber(NULL)\n");
	exit(0);
    }
    a->refCnt = 0;
}
...
{
    if (a == NULL) {
	fprintf(stderr, "incRefcntNumber(NULL)\n");
	exit(0);
    }
    a->refCnt++;
}
...
{
    if (a == NULL) {
	fprintf(stderr, "decRefcntNumber(NULL)\n");
	exit(0);
    }
    a->refCnt--;
}
...
{
    if (a == NULL) {
	fprintf(stderr, "printNumber(NULL)\n");
	exit(0);
    }
    switch (a->type) {
    case REAL:
...
Number * sinNumber(Number *a)
{
    ...
    if (a == NULL) {
	fprintf(stderr, "sin Number(NULL)\n");
	exit(0);
    }
...
Number * cosNumber(Number *a)
{
    ...
    if (a == NULL) {
	fprintf(stderr, "cos Number(NULL)\n");
	exit(0);
    }
...
Number * tanNumber(Number *a){
    ...
    if (a == NULL) {
	fprintf(stderr, "tan Number(NULL)\n");
	exit(0);
    }
...
Number * asinNumber(Number *a)
{
    ...
    if (a == NULL) {
	fprintf(stderr, "asin Number(NULL)\n");
	exit(0);
    }

    switch (a->type) {
    case REAL:
	if (1 == cmpReal(a->data, realOne) || -1 == cmpReal(a->data, realMOne)){
...
Number * acosNumber(Number *a)
{
    ...
    if (a == NULL) {
	fprintf(stderr, "acos Number(NULL)\n");
	exit(0);
    }
...
Number * atanNumber(Number *a)
{
    ...
    if (a == NULL) {
	fprintf(stderr, "atan Number(NULL)\n");
	exit(0);
    }
...
Number * powNumber(Number *a, Number *b)
{
	...
	switch (b->type) {
	case REAL:
	    if (-1 == cmpReal(a->data, realZero) && !isIntReal(b->data)){
		/* do complex math */
		c->type = COMPLEX;
		c1 = setCmplxReal(newCmplx(), a->data, realZero);
